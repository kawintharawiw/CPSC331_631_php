...
$create_table_sql = "create table shippers (shippersid varchar(20),  companyname varchar(20), phone varchar(20))";
...
if (strlen($act) == 0 || $act == "del" || $act == "edit") {
  ...
  while ($o = mysql_fetch_object($r)) {
    if (isset($_REQUEST{'shippersid '}) && $_REQUEST{'shippersid '}  == $o->shippersid ) $chg = " style='background-color:#f9f9f9"; else $chg = " readonly style='background-color:#ffffdd";
    echo "<tr><form action='' method=post>
      <td><input name=shippersid size=5 value='". $o->shippersid  . "' style='background-color:#dddddd' readonly></td>
      ...
      <td>";
    if (isset($_REQUEST{'shippersid'}) && $_REQUEST{'shippersid'} == $o->productid) {
  ...
  echo "<tr><form action='' method=post><td><input name=shippersid size=5></td><td><input name=companyname size=40></td><td><input name=phone size=20></td><td><input type=submit name=action value='add' style='height:26'></td></tr>
  </form></table>";
...
# ส่วนเพิ่มข้อมูล
if ($act == "add") {
  $q  = "insert into shippers values('". $_REQUEST{'shippersid'} . "','". $_REQUEST{'companyname'} . "','". $_REQUEST{'phone'} . "')";
  $conn = mysql_connect("$host","$user","$password");
...
# ส่วนลบข้อมูล
if ($act == "del : confirm") {
  $q  = "delete from shippers where shippersid ='". $_REQUEST{'shippersid'} . "'";
  $conn = mysql_connect("$host","$user","$password");
  $r = mysql_db_query($db,$q);   
...
# ส่วนแก้ไขข้อมูล
if ($act == "edit : confirm") {
  $q  = "update $tb set companyname ='". $_REQUEST{'companyname'} . "', phone ='". $_REQUEST{'phone'} . "' where shippersid =" . $_REQUEST{'shippersid'};
  $conn = mysql_connect("$host","$user","$password");
  $r = mysql_db_query($db,$q);   
